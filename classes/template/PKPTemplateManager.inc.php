...
class PKPTemplateManager extends Smarty {
	...
	function PKPTemplateManager($request = null) {
		...
		$this->setTemplateDir($baseDir . DIRECTORY_SEPARATOR . 'templates');
		import('lib.pkp.classes.template.PKPTemplateResource');
		$pkpTemplateDir = $baseDir . DIRECTORY_SEPARATOR . 'lib' . DIRECTORY_SEPARATOR . 'pkp' . DIRECTORY_SEPARATOR . 'templates';
		$this->pkpTemplateResource = new PKPTemplateResource($pkpTemplateDir);

		$this->addTemplateDir($pkpTemplateDir);

		$this->setCompileDir($cachePath . DIRECTORY_SEPARATOR . 't_compile');
		$this->setConfigDir($cachePath . DIRECTORY_SEPARATOR . 't_config');
		$this->setCacheDir($cachePath . DIRECTORY_SEPARATOR . 't_cache');
	...
	function initialize() {
		...
		$this->registerPlugin('modifier','get_value', array($this, 'smartyGetValue'));
		$this->registerPlugin('modifier','strip_unsafe_html', array('String', 'stripUnsafeHtml'));
		$this->registerPlugin('modifier','String_substr', array('String', 'substr'));
		...
		$this->registerPlugin('function','icon', array($this, 'smartyIcon'));
		$this->registerPlugin('function','sort_heading', array($this, 'smartySortHeading'));
		$this->registerPlugin('function','sort_search', array($this, 'smartySortSearch'));
		$this->registerPlugin('function','assign_mailto', array($this, 'smartyAssignMailto'));
		$this->registerPlugin('function','display_template', array($this, 'smartyDisplayTemplate'));
		$this->registerPlugin('modifier','truncate', array($this, 'smartyTruncate'));
		...
		$this->registerResource('core', $this->pkpTemplateResource);
	...
	function fetch($resource_name, $cache_id = null, $compile_id = null) {
		...
		if (!empty($this->javaScripts)) {
			$baseUrl = $this->getTemplateVars('baseUrl');
			...
			$additionalHeadData = $this->getTemplateVars('additionalHeadData');
		...
		return parent::fetch($resource_name, $cache_id, $compile_id);
	}
	 ...
	 */
	function display($template, $sendContentType = null, $hookName = null) {
		// Set the defaults
		...
		if (!HookRegistry::call($hookName, array($this, &$template, &$sendContentType, &$charset, &$output))) {
			...
			echo $this->fetch($template, null, null);
		} else {
	...
	function smartyTranslate($params, $smarty) {
		if (isset($params) && !empty($params)) {
			...
			if (isset($params['assign'])) {
				// Caller specified that the result should be assigned
	...
	function smartyIterate($params, $content, $smarty, &$repeat) {
		...
		if (isset($params['key'])) {
			...
			$smarty->assignByRef($params['item'], $value);
			$smarty->assignByRef($params['key'], $key);
		} else {
			$smarty->assignByRef($params['item'], $iterator->next());
		}
		return $content;
	}
	...
	function smartyIcon($params, $smarty) {
		if (isset($params) && !empty($params)) {
			...
			if (isset($params['name'])) {
				...
				$iconHtml = '<img src="' . $smarty->getTemplateVars('baseUrl') . '/' . $params['path'];
	 ...
	 * - assign: Variable to assign the resulting URL to (optional)
	 */
	function smartyUrl($parameters, $smarty) {
		...
		$result = $dispatcher->url($this->request, $router, $context, $handler, $op, $path, $parameters, $anchor, !isset($escape) || $escape);

		if (isset($parameters['assign'])) {
			// Caller specified that the result should be assigned
			$smarty->assign($parameters['assign'], $result);
		} else {
			return $result;
		}
	}
	...
	function smartySortHeading($params, $smarty) {
		if (isset($params) && !empty($params)) {
			...
			$sortDirection = $smarty->getTemplateVars('sortDirection');
			$sort = $smarty->getTemplateVars('sort');
	...
	function smartyLoadUrlInDiv($params, $smarty) {
		// Required Params
		if (!isset($params['url'])) {
			trigger_error("url parameter is missing from load_url_in_div");
		}
		if (!isset($params['id'])) {
			trigger_error("id parameter is missing from load_url_in_div");
		}
		// clear this variable, since it appears to carry over from previous load_url_in_div template assignments.
		$this->clearAssign(array('inDivClass'));
