...
class FormBuilderVocabulary {
	...
	function _smartyFBVTextArea($params, &$smarty) {
		...
		$textAreaParams = '';
		$smarty->clearAssign(array('FBV_label_content', 'FBV_disabled', 'FBV_readonly', 'FBV_multilingual', 'FBV_name', 'FBV_value', 'FBV_height', 'FBV_uniqId', 'FBV_rows', 'FBV_cols', 'FBV_rich', 'FBV_variables'));
		foreach ($params as $key => $value) {
			switch ($key) {
				case 'name':
				case 'value':
