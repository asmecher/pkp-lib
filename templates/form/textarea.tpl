	...
	<span id="{$FBV_name|escape}-localization-popover-container{$uniqId}" class="localization_popover_container">
		{strip}
			<textarea id="{$FBV_id|escape}-{$formLocale|escape}{$uniqId}" {$FBV_textAreaParams}
				...
				class="localizable {$FBV_class} {$FBV_height}{if $FBV_validation} {$FBV_validation|escape}{/if}{if $formLocale != $currentLocale} locale_{$formLocale|escape}{/if}{if $FBV_rich && empty($FBV_disabled)} richContent{/if}"
			...
			<div class="localization_popover">
				{foreach from=$formLocales key=thisFormLocale item=thisFormLocaleName}{if $formLocale != $thisFormLocale}
					...
					<textarea id="{$FBV_id|escape}-{$thisFormLocale|escape}{$uniqId}" {$FBV_textAreaParams}
						...
						class="flag flag_{$thisFormLocale|escape} {$FBV_class} {$FBV_height}{if $FBV_rich && empty($FBV_disabled)} richContent{/if}"
						{if !empty($FBV_disabled)} disabled="disabled"{/if}
						{if !empty($FBV_readonly)} readonly="readonly"{/if}
						{if $FBV_variables} data-variables="{$FBV_variables|@json_encode|escape:"url"}"{/if}
...
{else}
	...
	{if $FBV_rich && !empty($FBV_disabled)}
		{if !empty($FBV_multilingual)}{$FBV_value[$formLocale]|strip_unsafe_html}{else}{$FBV_value|strip_unsafe_html}{/if}
	{else}
		<textarea {$FBV_textAreaParams}
			class="{$FBV_class} {$FBV_height}{if $FBV_validation} {$FBV_validation|escape}{/if}{if $FBV_rich && empty($FBV_disabled)} richContent{/if}"
			{if !empty($FBV_disabled)} disabled="disabled"{/if}
			{if !empty($FBV_readonly)} readonly="readonly"{/if}
			{if $FBV_variables} data-variables="{$FBV_variables|@json_encode|escape:"url"}"{/if}
			...
			rows="{$FBV_rows|escape}"
			cols="{$FBV_cols|escape}"
			id="{$FBV_id|escape}{$uniqId}">{if !empty($FBV_multilingual)}{$FBV_value[$formLocale]|escape}{else}{$FBV_value|escape}{/if}</textarea>
	{/if}
