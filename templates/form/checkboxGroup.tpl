...
{if !empty($FBV_required)}{assign var="required" value="required"}{/if}
...
<li{if $FBV_layoutInfo} class="{$FBV_layoutInfo}"{/if}>
	<input type="checkbox" id="{$FBV_id|escape}-{$smarty.foreach.checkbox.index}" name="{$FBV_id|escape}[]"{$FBV_checkboxParams} class="field checkbox{if !empty($FBV_required)} required{/if}"{if $FBV_checked} checked="checked"{/if}{if $FBV_validation}{/if} value="{$FBV_value|escape}"{if !empty($FBV_disabled)} disabled="disabled"{/if}/>
